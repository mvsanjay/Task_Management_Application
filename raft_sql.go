...
func InitDB() *sql.DB {
	db, err := sql.Open("mysql", "root:Sql123...@tcp(127.0.0.1:3306)/task_manager")
	if err != nil {
...
func handleTaskByID(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodDelete:
		...
		json.NewEncoder(w).Encode(map[string]interface{}{"success": true})
	case http.MethodPut: // Add this case for handling PUT requests
		var task Task
		err := json.NewDecoder(r.Body).Decode(&task)
		if err != nil {
			http.Error(w, "Bad request", http.StatusBadRequest)
		...
		task.ID = id // Ensure the task ID matches the requested ID
